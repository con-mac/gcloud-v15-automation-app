...
function Search-WebApps {
    ...
    try {
        $ErrorActionPreference = 'SilentlyContinue'
        $output = az webapp list --resource-group $ResourceGroup --query "[].{Name:name}" -o json 2>&1
        $ErrorActionPreference = 'Stop'
        
        if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($output)) {
            try {
                $appsObjects = $output | ConvertFrom-Json
                if ($appsObjects) {
                    $apps = @()
                    # Handle both single object and array
                    if ($appsObjects -is [System.Array]) {
                        foreach ($obj in $appsObjects) {
                            if ($obj.Name -and $obj.Name.Trim().Length -gt 1) {
                                $apps += $obj.Name.Trim()
                            }
                        }
                    } else {
                        if ($appsObjects.Name -and $appsObjects.Name.Trim().Length -gt 1) {
                            $apps += $appsObjects.Name.Trim()
                        }
                    }
                    if ($apps.Count -gt 0) {
                        return $apps
        ...
        Write-Warning "Could not search for Web Apps: $_"
    }
    return @()
}
...
function Search-KeyVaults {
    ...
    try {
        ...
        # Use JSON query to filter by resource group
        $kvJson = az keyvault list --query "[?resourceGroup=='$ResourceGroup'].{Name:name}" -o json 2>&1
        $ErrorActionPreference = 'Stop'
        ...
        if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($kvJson)) {
            try {
                ...
                if ($kvObjects) {
                    $results = @()
                    # Handle both single object and array
                    if ($kvObjects -is [System.Array]) {
                        foreach ($obj in $kvObjects) {
                            if ($obj.Name -and $obj.Name.Trim().Length -gt 1) {
                                $results += $obj.Name.Trim()
                            }
                        }
                    } else {
                        if ($kvObjects.Name -and $kvObjects.Name.Trim().Length -gt 1) {
                            $results += $kvObjects.Name.Trim()
                        }
                    }
...
function Load-ExistingConfig {
    ...
    if (Test-Path $configPath) {
        Write-Info "Found existing configuration file. Loading values..."
        ...
        Write-Success "Loaded $($config.Count) configuration values from existing config file"
    }
...
function Start-Deployment {
    ...
    $existingConfig = Load-ExistingConfig
    
    # If UseExistingConfig flag is set and config exists, use it without prompts
    if ($UseExistingConfig -and $existingConfig.Count -gt 0) {
        Write-Info "Using existing configuration (--UseExistingConfig flag set)"
        Write-Info "All values will be loaded from config file. To override, edit config\deployment-config.env"
        Write-Host ""
        
        # Extract all values from config
        $script:RESOURCE_GROUP = $existingConfig.RESOURCE_GROUP
        $script:FUNCTION_APP_NAME = $existingConfig.FUNCTION_APP_NAME
        $script:WEB_APP_NAME = $existingConfig.WEB_APP_NAME
        $script:KEY_VAULT_NAME = $existingConfig.KEY_VAULT_NAME
        $script:SHAREPOINT_SITE_URL = $existingConfig.SHAREPOINT_SITE_URL
        $script:SHAREPOINT_SITE_ID = $existingConfig.SHAREPOINT_SITE_ID
        $script:APP_REGISTRATION_NAME = $existingConfig.APP_REGISTRATION_NAME
        $script:CUSTOM_DOMAIN = $existingConfig.CUSTOM_DOMAIN
        $script:STORAGE_ACCOUNT_NAME = $existingConfig.STORAGE_ACCOUNT_NAME
        $script:ACR_NAME = $existingConfig.ACR_NAME
        $script:IMAGE_TAG = $existingConfig.IMAGE_TAG
        $script:LOCATION = $existingConfig.LOCATION
        $script:PRIVATE_DNS_ZONE_NAME = $existingConfig.PRIVATE_DNS_ZONE_NAME
        $script:APP_INSIGHTS_NAME = $existingConfig.APP_INSIGHTS_NAME
        $script:VNET_NAME = $existingConfig.VNET_NAME
        $script:SUBNET_NAME = $existingConfig.SUBNET_NAME
        $script:CONFIGURE_PRIVATE_ENDPOINTS = $existingConfig.CONFIGURE_PRIVATE_ENDPOINTS
        
        Write-Success "Configuration loaded. Proceeding with deployment using existing values..."
        Write-Host ""
        
        # Get subscription info
        $subscription = az account show | ConvertFrom-Json
        $script:SUBSCRIPTION_ID = $subscription.id
        
        # Set defaults for missing values
        if ([string]::IsNullOrWhiteSpace($script:IMAGE_TAG)) {
            $script:IMAGE_TAG = "latest"
        }
        if ([string]::IsNullOrWhiteSpace($script:LOCATION)) {
            $script:LOCATION = "uksouth"
        }
        if ([string]::IsNullOrWhiteSpace($script:CONFIGURE_PRIVATE_ENDPOINTS)) {
            $script:CONFIGURE_PRIVATE_ENDPOINTS = "false"
        }
        
        # Validate required values
        $validationErrors = @()
        if ([string]::IsNullOrWhiteSpace($script:RESOURCE_GROUP)) {
            $validationErrors += "RESOURCE_GROUP is missing"
        }
        if ([string]::IsNullOrWhiteSpace($script:FUNCTION_APP_NAME)) {
            $validationErrors += "FUNCTION_APP_NAME is missing"
        }
        if ([string]::IsNullOrWhiteSpace($script:WEB_APP_NAME)) {
            $validationErrors += "WEB_APP_NAME is missing"
        }
        ...
        if ($validationErrors.Count -gt 0) {
            ...
            Write-Error "Please fix config\deployment-config.env and try again."
            exit 1
        }
        
        Write-Info "Skipping interactive prompts - using values from config file"
        Write-Info "To modify values, edit config\deployment-config.env and run again"
        Write-Host ""
        
        # Jump directly to deployment section (skip all prompts)
        # We'll set a flag and check it before each prompt section
        $script:SKIP_PROMPTS = $true
    } else {
        # Normal flow - use existing config as defaults for prompts
        Write-Info "Configuration mode: Interactive (existing values will be used as defaults)"
        if ($existingConfig.Count -gt 0) {
            Write-Info "Found existing config with $($existingConfig.Count) values - these will be used as defaults"
        }
        Write-Host ""
    }
    ...
    # Initialize SKIP_PROMPTS flag if not set
    if (-not $script:SKIP_PROMPTS) {
        $script:SKIP_PROMPTS = $false
    }
    
    # If skipping prompts, jump to deployment
    if ($script:SKIP_PROMPTS) {
        Write-Info "Skipping all configuration prompts - using existing config values"
        Write-Host ""
        # Jump to deployment section (after all prompts)
        # We'll find where deployment starts and jump there
    } else {
    ...
    $WEB_APP_NAME = Get-ResourceName -ResourceType "Web App" -DefaultName $defaultWebName -ExistingResources $existingWebApps -RandomSuffix $randomSuffix
    
    # Validate and ensure suffix is applied
    ...
    Write-Success "Selected Web App: $WEB_APP_NAME"
    ...
    $existingKvValue = if ($existingConfig.KEY_VAULT_NAME) { $existingConfig.KEY_VAULT_NAME } else { "" }
    $KEY_VAULT_NAME = Get-ResourceName -ResourceType "Key Vault" -DefaultName $defaultKvName -ExistingResources $existingKeyVaults -RandomSuffix $randomSuffix -ExistingConfigValue $existingKvValue
    ...
    Write-Success "Selected Key Vault: $KEY_VAULT_NAME"
